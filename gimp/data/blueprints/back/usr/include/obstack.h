// GGHASH:VCNVffGvQensxylSbtqyXD2a7RCQ_oU2C1nG7hxzV0j40000533a
#ifndef _OBSTACK_H
#define _OBSTACK_H 1
...
# endif
#define obstack_object_size(OBSTACK) __extension__ ({ struct obstack const *__o = (OBSTACK); (unsigned) (__o->next_free - __o->object_base); })
#define obstack_room(OBSTACK) __extension__ ({ struct obstack const *__o = (OBSTACK); (unsigned) (__o->chunk_limit - __o->next_free); })
...
#define obstack_empty_p(OBSTACK) __extension__ ({ struct obstack const *__o = (OBSTACK); (__o->chunk->prev == 0 && __o->next_free == __PTR_ALIGN ((char *) __o->chunk, __o->chunk->contents, __o->alignment_mask)); })
...
#define obstack_grow0(OBSTACK,where,length) __extension__ ({ struct obstack *__o = (OBSTACK); int __len = (length); if (__o->next_free + __len + 1 > __o->chunk_limit) _obstack_newchunk (__o, __len + 1); memcpy (__o->next_free, where, __len); __o->next_free += __len; *(__o->next_free)++ = 0; (void) 0; })
...
#define obstack_ptr_grow(OBSTACK,datum) __extension__ ({ struct obstack *__o = (OBSTACK); if (__o->next_free + sizeof (void *) > __o->chunk_limit) _obstack_newchunk (__o, sizeof (void *)); obstack_ptr_grow_fast (__o, datum); })
#define obstack_int_grow(OBSTACK,datum) __extension__ ({ struct obstack *__o = (OBSTACK); if (__o->next_free + sizeof (int) > __o->chunk_limit) _obstack_newchunk (__o, sizeof (int)); obstack_int_grow_fast (__o, datum); })
#define obstack_ptr_grow_fast(OBSTACK,aptr) __extension__ ({ struct obstack *__o1 = (OBSTACK); void *__p1 = __o1->next_free; *(const void **) __p1 = (aptr); __o1->next_free += sizeof (const void *); (void) 0; })
#define obstack_int_grow_fast(OBSTACK,aint) __extension__ ({ struct obstack *__o1 = (OBSTACK); void *__p1 = __o1->next_free; *(int *) __p1 = (aint); __o1->next_free += sizeof (int); (void) 0; })
#define obstack_blank(OBSTACK,length) __extension__ ({ struct obstack *__o = (OBSTACK); int __len = (length); if (__o->chunk_limit - __o->next_free < __len) _obstack_newchunk (__o, __len); obstack_blank_fast (__o, __len); (void) 0; })
...
#define obstack_free(OBSTACK,OBJ) __extension__ ({ struct obstack *__o = (OBSTACK); void *__obj = (OBJ); if (__obj > (void *) __o->chunk && __obj < (void *) __o->chunk_limit) __o->next_free = __o->object_base = (char *) __obj; else (__obstack_free) (__o, __obj); })
#else
...
#define obstack_ptr_grow(h,datum) ((((h)->next_free + sizeof (char *) > (h)->chunk_limit) ? (_obstack_newchunk ((h), sizeof (char *)), 0) : 0), obstack_ptr_grow_fast (h, datum))
#define obstack_int_grow(h,datum) ((((h)->next_free + sizeof (int) > (h)->chunk_limit) ? (_obstack_newchunk ((h), sizeof (int)), 0) : 0), obstack_int_grow_fast (h, datum))
