// GGHASH:VTaJbCZxM8MwDbTF.AkjLrT4sx2l20mbUh_FMqIX4DyE00002eed
...
#define MUTEX_INIT(m) STMT_START { *m = mutex_alloc(); if (*m) { mutex_init(*m); } else { Perl_croak_nocontext("panic: MUTEX_INIT [%s:%d]", __FILE__, __LINE__); } } STMT_END
#define MUTEX_LOCK(m) mutex_lock(*m)
#define MUTEX_UNLOCK(m) mutex_unlock(*m)
#define MUTEX_DESTROY(m) STMT_START { mutex_free(*m); *m = 0; } STMT_END
#define COND_INIT(c) STMT_START { *c = condition_alloc(); if (*c) { condition_init(*c); } else { Perl_croak_nocontext("panic: COND_INIT [%s:%d]", __FILE__, __LINE__); } } STMT_END
...
#define COND_DESTROY(c) STMT_START { condition_free(*c); *c = 0; } STMT_END
...
#define MUTEX_INIT(m) STMT_START { int _eC_; Zero((m), 1, perl_mutex); if ((_eC_ = pthread_mutex_init((m), pthread_mutexattr_default))) Perl_croak_nocontext("panic: MUTEX_INIT (%d) [%s:%d]", _eC_, __FILE__, __LINE__); } STMT_END
# else
...
#endif
#if (defined(__ALPHA) && (__VMS_VER >= 70000000)) || (defined(__alpha) && defined(__osf__) && !defined(__GNUC__))
